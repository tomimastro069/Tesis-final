...
* **`tarea` (`/tarea`, `/plan-capitulo`):** Planifica e implementa tareas individuales del plan de escalado hacia calificación sobresaliente (9,0 a 10,0) en `docs/plan/plan-v16-punt-9.md`, auditando el texto plano, garantizando cero invención de datos y elaborando el plan de implementación previo.
* **`tarea-hecha` (`/tarea-hecha`, `/tarea-echa`):** Cierra formalmente una tarea completada, actualiza la matriz de notas, el checklist de ejecución y el mini-walkthrough en `docs/plan/plan-v16-punt-9.md`, y genera la sugerencia de commit reglamentaria. Solo se ejecuta ante invocación explícita con `/`.
